...
/*
 ...
 public function settle_insert() {
    ...
    if(!empty($lastContractorRecord)){
        if($lastContractorRecord->contractor_invoice_id<20241){
            $insertData['contractor_invoice_id'] = 20241;
        }else{
 ...
 public function receiveInvoice(){
    if($_POST){
        ...
        if(!empty($maxInvoiceData)){
            $explodedvalues = explode('2024', $maxInvoiceData->received_invoice_id);
            $updatedValue = $explodedvalues[1]+1;
            $lastRecieveId = ($maxInvoiceData->received_invoice_id < 20241) ? 20241 : "2024".$updateValue;
        }else{
            $lastRecieveId = 20241;
        }
